...
export class PerfilUsuarioComponent implements OnInit {
  ...
  private readonly perfilKey = 'perfilCliente';
  private readonly enderecosKey = 'enderecosCliente';
  private readonly pagamentosKey = 'pagamentosCliente';
  private readonly historicoKey = 'historicoPedidos';
  ...
  carregarPerfil(): void {
    ...
    this.authService.buscarMeuPerfil().subscribe({
      ...
      error: () => {
        this.carregandoPerfil = false;
        this.carregarPerfilLocal();
        this.exibirMensagem('Não foi possível carregar o perfil. Faça login novamente.');
      }
    });
  }
  ...
  salvarPerfil(): void {
    ...
    if (!this.authService.logado()) {
      this.exibirMensagem('Você precisa estar logado para alterar os dados da conta.');
      return;
    }
    ...
    this.authService.atualizarPerfil(dto).subscribe({
      ...
      error: (erro) => {
        ...
        if (erro.status === 401) {
          this.exibirMensagem('Senha de confirmação incorreta.');
          return;
        }

        if (erro.status === 409) {
          this.exibirMensagem('Este usuário já está em uso.');
          return;
        }

        this.exibirMensagem('Não foi possível salvar os dados da conta.');
      }
    });
  }
  ...
  alterarSenha(): void {
    ...
    if (this.senhaForm.invalid) {
      ...
      if (this.senhaForm.hasError('senhasDiferentes')) {
        this.exibirMensagem('A nova senha e a confirmação não conferem.');
        return;
      }
    ...
    if (!this.authService.logado()) {
      this.exibirMensagem('Você precisa estar logado para alterar a senha.');
      return;
    }
    ...
    this.authService.alterarSenha(dto).subscribe({
      next: () => {
        this.exibirMensagem('Senha alterada com sucesso! Faça login novamente com a nova senha.');
      ...
      error: (erro) => {
        ...
        this.exibirMensagem('Não foi possível alterar a senha.');
      }
    });
  }
  ...
  sairDaConta(): void {
    this.authService.sair();
    this.exibirMensagem('Você saiu da sua conta.');
  ...
  comprarNovamente(pedido: PedidoHistorico): void {
    if (!pedido.itens || pedido.itens.length === 0) {
      this.exibirMensagem('Este pedido não possui itens para comprar novamente.');
      return;
    }
  ...
  adicionarEndereco(): void {
    if (this.enderecoForm.invalid) {
      ...
      this.exibirMensagem('Preencha os dados obrigatórios do endereço.');
      return;
    }
    ...
    this.exibirMensagem('Endereço adicionado com sucesso!');
  }
  ...
  definirEnderecoPrincipal(id: number): void {
    ...
    this.salvarEnderecos();
    this.exibirMensagem('Endereço principal atualizado.');
  }
  ...
  removerEndereco(id: number): void {
    ...
    this.salvarEnderecos();
    this.exibirMensagem('Endereço removido.');
  }
  ...
  adicionarPagamento(): void {
    if (this.pagamentoForm.invalid) {
      ...
      this.exibirMensagem('Preencha os dados obrigatórios do pagamento.');
      return;
    }
    ...
    if (tipo === 'Cartão' && numeroLimpo.length < 4) {
      this.exibirMensagem('Informe ao menos os últimos 4 dígitos do cartão.');
      return;
    }
