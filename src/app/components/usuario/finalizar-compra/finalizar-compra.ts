...
export class FinalizarCompraComponent implements OnInit {
  ...
  confirmarCompra(): void {
    ...
    const pedido = {
      id: Date.now(),
      usuario: this.authService.getUsernameLogado(),
    ...
    localStorage.setItem(this.getUltimoPedidoKey(), JSON.stringify(pedido));

    /*
      Mantido temporariamente para não quebrar a tela de resumo,
      caso ela ainda esteja lendo a chave antiga "ultimoPedido".
      O histórico do perfil já fica separado por usuário.
    */
    localStorage.setItem('ultimoPedido', JSON.stringify(pedido));

    const historico = this.buscarJson(this.getHistoricoKey());
    const historicoAtual = Array.isArray(historico) ? historico : [];

    historicoAtual.push(pedido);

    localStorage.setItem(this.getHistoricoKey(), JSON.stringify(historicoAtual));
  ...
  private getHistoricoKey(): string {
    return `historicoPedidos_${this.getUsuarioKey()}`;
  }

  private getUltimoPedidoKey(): string {
    return `ultimoPedido_${this.getUsuarioKey()}`;
  }

  private getUsuarioKey(): string {
